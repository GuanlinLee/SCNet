...
def rank(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba):
	...
	for p in range(0, max_trace_idx-min_trace_idx):
		...
		plaintext = metadata[min_trace_idx + p][0]
...
# Check a saved model against one of the dpa databases Attack traces
def check_model(model_file, dpa_database, num_traces=5000):
	check_file_exists(model_file)
	check_file_exists(dpa_database)
	# Load profiling and attack data and metadata from the dpa database
	(X_profiling, Y_profiling), (X_attack, Y_attack), (Metadata_profiling, Metadata_attack) = load_dpa(dpa_database, load_metadata=True)
	# Load model
	...
	plt.grid(True)
	plt.plot(x, y)
	np.save(model_file.split('/')[-1]+'_x.npy',x)
	np.save(model_file.split('/')[-1]+'_y.npy',y)
	plt.savefig('Performance of '+model_file.split('/')[-1]+' against '+dpa_database.split('/')[-1]+'.png',transparent=True,bbox_inches= 'tight')
...
to_check_all = [
	(dpa_trained_models_folder + "best_model_desync0_SCNet_v2.h5", dpa_databases_folder + "DPA.h5"),
