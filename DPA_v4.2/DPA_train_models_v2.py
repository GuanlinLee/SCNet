...
#1-1
	...
	conv1 = AveragePooling1D(2, 2)(conv1)
...
def train_model(X_profiling, Y_profiling, model, save_file_name=None, epochs=150, batch_size=50,name=None):
	if save_file_name:
		...
		decay=ReduceLROnPlateau(monitor='val_acc',patience=5,factor=0.66)
		...
		decay = ReduceLROnPlateau(monitor='val_acc', patience=8,factor=0.6)
		callbacks = [save_model,es,decay]
		# Get the input layer shape
		input_layer_shape = model.get_layer(index=0).input_shape
		# Sanity check
...
train_model(X_profiling, Y_profiling, cnn_best_model, epochs=75, batch_size=200,name='desync0_SCNet_v2')
