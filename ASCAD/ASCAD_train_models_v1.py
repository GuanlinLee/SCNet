...
def train_model(X_profiling, Y_profiling, model, save_file_name=None, epochs=150, batch_size=100,name=None):
	if save_file_name:
		...
		decay = ReduceLROnPlateau(monitor='val_loss', patience=6,factor=0.15)
		callbacks = [save_model,es,decay]
		# Get the input layer shape
		input_layer_shape = model.get_layer(index=0).input_shape
		# Sanity check
...
train_model(X_profiling, Y_profiling, cnn_best_model, epochs=75, batch_size=100,name='desync0_SCNet_v1')
#### Desync = 50
cnn_best_model = SCNet_v1()
train_model(X_profiling_desync50, Y_profiling_desync50, cnn_best_model,  epochs=75, batch_size=100,name='desync50_SCNet_v1')
#### Desync = 100
cnn_best_model = SCNet_v1()
train_model(X_profiling_desync100, Y_profiling_desync100, cnn_best_model,  epochs=75, batch_size=100,name='desync100_SCNet_v1')
